			...
			if (setinfo) {
				...
				if (n.announce) {
					await XeonBotInc.sendMessage(n.id, {
						text: 'group telah ditutup Admin',
						contextInfo: {
							externalAdReply: {
								title: 'Group Closed',
								mediaType: 1,
					...
					await XeonBotInc.sendMessage(n.id, {
						text: 'group telah di buka Admin',
						contextInfo: {
							externalAdReply: {
								title: 'Group Open',
								mediaType: 1,
					...
					await XeonBotInc.sendMessage(n.id, {
						text: 'hanya admin yang bisa update info',
						contextInfo: {
							externalAdReply: {
					...
					await XeonBotInc.sendMessage(n.id, {
						text: 'sekarang semua bisa update info',
						contextInfo: {
							externalAdReply: {
								title: 'Group Update Info',
								mediaType: 1,
					...
					await XeonBotInc.sendMessage(n.id, {
						text: 'group telah dirubah ' + n.subject,
						contextInfo: {
							externalAdReply: {
	...
	try {
		...
		if (welcome) {
			...
			for (let n of participants) {
                ...
                xeonbody = `@${xeonName.split("@")[0]} selamatdatang sekarang kita pren yah`
...
await XeonBotInc.relayMessage(id, msgs.message, {})
	            ...
	            const xeonmembers = metadata.participants.length
					xeonbody = `${xeonName.split("@")[0]} kita bukan fren lgi
  `
let msgs = generateWAMessageFromContent(id, {
  viewOnceMessage: {
    message: {
